...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (form) {
            ...
            // Add a submit handler to log form data
            form.addEventListener('submit', function(e) {
                ...
                if (csrfToken) {
                    console.log('CSRF token is present:', csrfToken);
                    console.log('CSRF token in form:', formData.get('_token'));
                } else {
                    console.warn('CSRF token is missing in meta tag!');
                }
            });
        } else {
            console.error('Form with ID "createEventForm" not found!');
        }
    });
</script>
<div class="min-h-screen bg-gradient-to-b from-white to-gray-50 py-8">
    <div class="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-2xl font-semibold text-gray-800">Add New Event</h1>
                <p class="text-sm text-gray-500 mt-1">Create a new club event</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.events.store') }}" method="POST" enctype="multipart/form-data" class="p-6" id="createEventForm">
                @csrf
                    ...
                    <div class="grid grid-cols-1 gap-x-6 gap-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-300 focus:ring focus:ring-red-200 focus:ring-opacity-50 @error('description') border-red-300 focus:border-red-500 focus:ring-red-500 @enderror">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="event_type" id="event_type"
                                ...
                                <option value="Club ceremony" {{ old('event_type') == 'Club ceremony' ? 'selected' : '' }}>Club ceremony</option>
                                <option value="Technical workshop" {{ old('event_type') == 'Technical workshop' ? 'selected' : '' }}>Technical workshop</option>
                                <option value="Club meeting" {{ old('event_type') == 'Club meeting' ? 'selected' : '' }}>Club meeting</option>
                                <option value="Social gathering" {{ old('event_type') == 'Social gathering' ? 'selected' : '' }}>Social gathering</option>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="start_date_time" id="start_date_time" value="{{ old('start_date_time') }}"
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="end_date_time" id="end_date_time" value="{{ old('end_date_time') }}"
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="location_details" id="location_details" value="{{ old('location_details') }}" placeholder="Building, room, etc."
                    ...
                    <div class="grid grid-cols-1 gap-x-6 gap-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="3" placeholder="Additional information about the event"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-300 focus:ring focus:ring-red-200 focus:ring-opacity-50 @error('notes') border-red-300 focus:border-red-500 focus:ring-red-500 @enderror">{{ old('notes') }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <input type="file" name="event_image" id="event_image" accept=".jpg,.jpeg"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-300 focus:ring focus:ring-red-200 focus:ring-opacity-50 @error('event_image') border-red-300 focus:border-red-500 focus:ring-red-500 @enderror">
                            <p class="mt-1 text-xs text-gray-500">Upload a JPG image for the event (optional)</p>
                            @error('event_image')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="flex items-center justify-end space-x-3 mt-8">
                    <a href="{{ route('omcms.events') }}" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors duration-200">
                        Cancel
                    </a>
                    <button type="submit" class="px-4 py-2 bg-red-600 hover:bg-red-700 rounded-md text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors duration-200">
                        Create Event
                    </button>
                </div>

                <!-- Add hidden field for redirect URL -->
                <input type="hidden" name="redirect_url" value="{{ url('/omcms/events') }}" />
            </form>
        </div>
    </div>
</div>
