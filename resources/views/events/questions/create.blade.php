...
<div class="min-h-screen bg-gradient-to-b from-white to-gray-50 py-8">
    <div class="max-w-3xl mx-auto px-4 sm:px-6">
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-2xl font-semibold text-gray-800">Add Evaluation Question</h1>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('events.questions.store', $event) }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="question_text" id="question_text" value="{{ old('question_text') }}" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-300 focus:ring focus:ring-red-200 focus:ring-opacity-50" placeholder="Enter your question here...">
                    @error('question_text')
                ...
                <div class="mb-6">
                    ...
                    <select name="question_type" id="question_type" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-300 focus:ring focus:ring-red-200 focus:ring-opacity-50">
                        <option value="rating" {{ old('question_type') === 'rating' ? 'selected' : '' }}>Rating (1-5 scale)</option>
                        <option value="text" {{ old('question_type') === 'text' ? 'selected' : '' }}>Text (Free response)</option>
                    </select>
                    @error('question_type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    <div class="flex items-center">
                        <input type="checkbox" name="is_required" id="is_required" value="1" {{ old('is_required') ? 'checked' : '' }} class="h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300 rounded">
